...
import {
  DEFAULT_VERSION_ID,
  getVersionById,
  loadStoredDefaultVersionId,
  persistDefaultVersionId,
  PROMPT_VERSIONS,
  type PromptVersion,
} from "./aiNarrativeVersions";

type WrapTab = "prompt" | "markdown";
type DrawerView = "edit" | "history" | "version";
...
export function AiNarrativeReportPage() {
  ...
  const [drawerView, setDrawerView] = useState<DrawerView>("edit");
  const [tab, setTab] = useState<WrapTab>("prompt");
  const [defaultVersionId, setDefaultVersionId] = useState(() => loadStoredDefaultVersionId());
  const [activeVersionId, setActiveVersionId] = useState(() => loadStoredDefaultVersionId());
  const [previewVersionId, setPreviewVersionId] = useState<string | null>(null);
  const activeVersion = getVersionById(activeVersionId);
  const [prompt, setPrompt] = useState(() => getVersionById(loadStoredDefaultVersionId()).prompt);
  const [markdown, setMarkdown] = useState(() => getVersionById(loadStoredDefaultVersionId()).markdown);
  const [reportVersionId, setReportVersionId] = useState(() => loadStoredDefaultVersionId());
  const [isLoading, setIsLoading] = useState(() => searchParams.get("generate") === "1");
  ...
  const report = getVersionById(reportVersionId).report;
  const previewVersion = previewVersionId ? getVersionById(previewVersionId) : null;
  ...
  useEffect(() => {
    ...
    navigate(AI_NARRATIVE_PIN.path, { replace: true });
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (drawerView === "version") setDrawerView("history");
        else if (drawerView === "history") setDrawerView("edit");
  ...
  function applyVersionToEditor(version: PromptVersion) {
    setActiveVersionId(version.id);
    setPrompt(version.prompt);
    setMarkdown(version.markdown);
  }

  function regenerateWithVersion(versionId: string) {
  ...
  function handleGenerate() {
    setReportVersionId(activeVersionId);
    setDrawerOpen(false);
    setDateRange(last7Days());
    setIsLoading(true);
  }
  ...
  function handleRestore() {
    const version = getVersionById(defaultVersionId);
    applyVersionToEditor(version);
  }

  function handleDownload(text?: string) {
    const blob = new Blob([text ?? markdown], { type: "text/markdown;charset=utf-8" });
  ...
  function handleSetAsDefault() {
    ...
    persistDefaultVersionId(version.id);
    applyVersionToEditor(version);
    regenerateWithVersion(version.id);
    setDrawerView("edit");
    setPreviewVersionId(null);
    setDrawerOpen(false);
  }
  ...
  const historyVersions = [...PROMPT_VERSIONS].reverse();

  return (
    <>
      <ReportPageShell
        title="Partner Dashboard"
        subtitle={`AI-generated Brand report · ${rangeLabel} · ${getVersionById(reportVersionId).label}`}
        actions={
        ...
        {isLoading ? (
          ...
          >
            ...
            >
              ...
              >
                {report.takeawayBody} Reporting window: {rangeLabel}.
              </div>
            </div>
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      fontSize: 12,
                      fontWeight: 600,
                      color: k.up === false ? "var(--destructive, #dc2626)" : "var(--success, #16a34a)",
                    }}
                  >
                    {k.delta}
                  </div>
            ...
            <Section title="01 · Exposure" headline={report.exposureHeadline}>
              <div className="grid grid-cols-2 gap-[12px]">
                <ChartCard label="Impressions trend">
                  <div className="flex items-end gap-[4px] h-[120px]">
                    {report.impressionBars.map((h, i) => (
                      <div
                        key={i}
            ...
            <Section title="02 · Attention" headline={report.attentionHeadline}>
              <div
            ...
            <Section title="03 · Conversion" headline={report.conversionHeadline}>
              <div
      ...
      {drawerOpen && (
        <>
          ...
          >
            ...
            {/* Slideout Top Bar — Figma 9714:96338 states */}
            <div
            ...
            >
              {drawerView === "edit" ? (
                ...
                <button
                  ...
                  aria-label="Back"
                  onClick={() =>
                    drawerView === "version" ? setDrawerView("history") : setDrawerView("edit")
                  }
                  className="size-[40px] rounded-[var(--radius-button,9999px)] flex items-center justify-center cursor-pointer shrink-0 border-0"
                ...
                {drawerView !== "history" && (
                  ...
                  >
                    {drawerView === "version"
                      ...
                      : activeVersionId === DEFAULT_VERSION_ID && defaultVersionId === DEFAULT_VERSION_ID
            ...
            {drawerView === "history" ? (
              <div className="flex-1 min-h-0 overflow-auto p-[32px] flex flex-col gap-[24px]">
                {historyVersions.map((version) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center justify-between pb-[20px] cursor-pointer border-0 bg-transparent text-left px-0"
                      style={{ borderBottom: "1px solid var(--border)" }}
                    >
                      <div className="flex flex-col gap-[2px] min-w-0">
                        <div className="flex items-center gap-[8px]">
                          ...
                          {isDefault && (
                            <span
                              className="size-[16px] rounded-full inline-flex items-center justify-center shrink-0"
                ...
                <div className="flex-1 min-h-0 overflow-auto p-[32px]">
                  ...
                  >
                    <div className="flex gap-[8px] items-start w-full">
                      ...
                      <IconCircleButton ariaLabel="Prompt history" title="History" onClick={openHistory}>
                    ...
                    <div className="flex-1 min-h-0 flex flex-col justify-between relative w-full">
                      {drawerView === "version" && previewVersion ? (
                        <div
                          className="w-full flex-1 min-h-[280px] overflow-auto font-['Sarabun',sans-serif] whitespace-pre-wrap"
                          style={{
                            color: "#F5F5F5",
                            fontSize: 13,
                            lineHeight: "normal",
                        ...
                        >
                          {tab === "prompt" ? previewVersion.prompt : previewVersion.markdown}
                        </div>
                      ) : (
                        <textarea
                  ...
                  {drawerView === "version" ? (
                    <>
                      <button
                        ...
                        disabled={previewVersionId === defaultVersionId}
                        className="h-[40px] min-w-[65px] px-[16px] cursor-pointer font-['Sarabun',sans-serif] border-0 shrink-0 disabled:opacity-60"
                        style={{
                          borderRadius: "var(--radius-button, 9999px)",
                          background: "var(--button-primary, #1D66DE)",
                      ...
                      >
                        {previewVersionId === defaultVersionId ? "Default" : "Set as default"}
                      </button>
                      <button
                        type="button"
                        onClick={() => handleDownload(previewVersion?.markdown)}
