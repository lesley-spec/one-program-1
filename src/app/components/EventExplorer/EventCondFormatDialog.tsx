/*
  EventCondFormatDialog — conditional formatting for V2.

  Same pattern as the V1 dialog, scoped to V2 measure refs.
*/

import { useEffect, useState } from "react";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogDescription } from "../ui/dialog";
import type { ExplorerAction, ExplorerState } from "../../event-explorer/state";
import { type ColumnRef, fieldOf, refKey } from "../../event-explorer/schema";
...
export function EventCondFormatDialog({ state, dispatch, initialRef, onClose }: Props) {
  ...
  const [val, setVal] = useState(String(existing?.value ?? 1000));
  const [val2, setVal2] = useState(String(existing?.value2 ?? ""));
  ...
  }, [existing?.ref.fieldId, existing?.ref.sourceId]);
        ...
        <div className="flex flex-col gap-[16px]">
          ...
          <div className="flex flex-col gap-[6px]">
            ...
            <div className="flex flex-wrap gap-[6px]">
              {(["<", ">", "between"] as const).map((o) => {
                const active = op === o;
                const lbl = o === ">" ? "> More than" : o === "<" ? "< Less than" : "↔ Between";
                return (
                  <button
                    key={o}
                    onClick={() => setOp(o)}
          ...
          <div className="flex gap-[12px]">
            ...
            {op === "between" && (
              <div className="flex-1 flex flex-col gap-[6px]">
                <Label>To</Label>
                <NumInput value={val2} onChange={setVal2} />
              </div>
            )}
          </div>
...
function Label({ children }: { children: React.ReactNode }) {
  return <span style={{ fontFamily: FONT, fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--foreground)" }}>{children}</span>;
}
  ...
  return (
    <input
      ...
      style={{
        ...
        border: "1px solid var(--border)",
        background: "var(--card)",
        fontFamily: FONT, fontSize: "var(--text-base)", color: "var(--foreground)",
      }}
    />
  );
}
function ColorField({ label, value, onChange, presets }: { label: string; value: string; onChange: (v: string) => void; presets: string[] }) {
  return (
    <div className="flex flex-col gap-[6px]">
      <span style={{ fontFamily: FONT, fontSize: 11, fontWeight: 600, color: "var(--muted-foreground)", letterSpacing: "0.4px", textTransform: "uppercase" }}>{label}</span>
      <div className="flex items-center gap-[8px]">
        <div style={{ width: 24, height: 24, borderRadius: "var(--radius-button)", background: value, border: "1px solid var(--border)" }} />
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
