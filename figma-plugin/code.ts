...
interface InsertTablePayload {
  rows: number;
  columns: { header: string; type: ColumnType }[];
}
...
interface UIMessage {
  type: "insert-chart" | "insert-table";
  payload: InsertChartPayload | InsertTablePayload;
}
...
interface DesignSystemContext {
  ...
  vnextModeId: string | null;
  tableComponent: ComponentNode | ComponentSetNode | null;
}
...
async function initialize(): Promise<void> {
  try {
    ...
    if (warnings.length) summary.push(`(${warnings.join("; ")})`);
...
figma.ui.onmessage = async (raw: unknown) => {
  ...
  try {
    if (msg.type === "insert-chart") {
      await handleInsertChart(msg.payload as InsertChartPayload);
    } else if (msg.type === "insert-table") {
      await handleInsertTable(msg.payload as InsertTablePayload);
    }
  } catch (err) {
...
async function resolveStyleguideV2(): Promise<{
  collection: VariableCollection | null;
  vnextModeId: string | null;
  warnings: string[];
}> {
  ...
  const libCollection = libraryCollections.find((c) =>
    STYLEGUIDE_LIBRARY_NAME_MATCHERS.some(
      (re) => re.test(c.libraryName || "") || re.test(c.name || ""),
    ),
  );
  ...
  if (!collection) {
    ...
    collection =
      localCollections.find((c) =>
        STYLEGUIDE_LIBRARY_NAME_MATCHERS.some((re) => re.test(c.name)),
      ) || null;
  }

  let vnextModeId: string | null = null;
  if (collection) {
    const mode = collection.modes.find((m) => VNEXT_MODE_MATCHERS.some((re) => re.test(m.name)));
    vnextModeId = mode ? mode.modeId : null;
    if (!mode) warnings.push(`no Vnext mode on ${collection.name}`);
  }

  return { collection, vnextModeId, warnings };
}

async function resolveTableComponent(): Promise<ComponentNode | ComponentSetNode | null> {
  // 1. Explicit key wins.
...
function findLocalComponent(root: BaseNode): ComponentNode | ComponentSetNode | null {
  if (!("findAllWithCriteria" in root)) return null;
  const all = (root as DocumentNode).findAllWithCriteria({
...
async function findInstanceMainComponent(): Promise<ComponentNode | ComponentSetNode | null> {
...
function createReportFrame(title: string, width: number, height: number): FrameNode {
...
function placeAtViewport(frame: FrameNode): void {
...
function makeText(
  characters: string,
  opts?: { weight?: "Regular" | "Medium" | "Semi Bold"; size?: number; color?: RGB },
): TextNode {
...
async function handleInsertChart(payload: InsertChartPayload): Promise<void> {
  await loadFonts();
  const FRAME_W = 720;
  const FRAME_H = 460;
...
function humanizeChartTitle(t: ChartType): string {
  return t.replace(/([A-Z])/g, " $1").trim();
}

function makeMockSeries(seriesCount: number, pointCount: number): number[][] {
  const series: number[][] = [];
...
function drawChart(plot: FrameNode, type: ChartType, data: number[][]): void {
  const W = plot.width;
  const H = plot.height;
  ...
  switch (type) {
    ...
    case "AreaChart":
      drawAxes(plot, W, H);
      data.forEach((s, i) =>
        drawArea(plot, s, SERIES_COLORS[i % SERIES_COLORS.length], W, H),
      );
      break;
    case "PieChart":
    ...
    case "ScatterChart":
      drawAxes(plot, W, H);
      data.forEach((s, i) =>
        drawScatter(plot, s, SERIES_COLORS[i % SERIES_COLORS.length], W, H),
      );
      break;
    case "ComposedChart":
      drawAxes(plot, W, H);
      drawBars(plot, [data[0]], W, H);
      if (data[1]) drawLine(plot, data[1], SERIES_COLORS[1], W, H);
      break;
    case "RadarChart":
...
const AXIS_COLOR: RGB = { r: 0.78, g: 0.78, b: 0.82 };
const GRID_COLOR: RGB = { r: 0.92, g: 0.92, b: 0.94 };
const PADDING_L = 36;
const PADDING_R = 16;
const PADDING_T = 16;
const PADDING_B = 28;

function plotArea(W: number, H: number): { x: number; y: number; w: number; h: number } {
  return {
    x: PADDING_L,
    y: PADDING_T,
...
function drawAxes(plot: FrameNode, W: number, H: number): void {
...
function drawLine(plot: FrameNode, series: number[], color: RGB, W: number, H: number): void {
  const a = plotArea(W, H);
...
function seriesToPath(series: number[], a: { x: number; y: number; w: number; h: number }): string {
  let d = "";
  series.forEach((v, i) => {
...
function drawBars(plot: FrameNode, dataSets: number[][], W: number, H: number): void {
  const a = plotArea(W, H);
...
function drawArea(plot: FrameNode, series: number[], color: RGB, W: number, H: number): void {
  const a = plotArea(W, H);
  let d = "";
  series.forEach((v, i) => {
    const x = a.x + (a.w * i) / Math.max(1, series.length - 1);
...
function drawPie(plot: FrameNode, values: number[], W: number, H: number): void {
...
function drawScatter(plot: FrameNode, series: number[], color: RGB, W: number, H: number): void {
  const a = plotArea(W, H);
  series.forEach((v, i) => {
...
function drawRadar(plot: FrameNode, data: number[][], W: number, H: number): void {
  const cx = W / 2;
  const cy = H / 2;
...
function drawRadialBars(plot: FrameNode, values: number[], W: number, H: number): void {
  const cx = W / 2;
  const cy = H / 2;
...
function ringArcPath(cx: number, cy: number, rIn: number, rOut: number, a0: number, a1: number): string {
  const large = a1 - a0 > Math.PI ? 1 : 0;
...
function drawTreemap(plot: FrameNode, values: number[], W: number, H: number): void {
...
function drawTreeRect(plot: FrameNode, x: number, y: number, w: number, h: number, depth: number): void {
  const r = figma.createRectangle();
  r.x = x + 2;
  r.y = y + 2;
...
function drawFunnel(plot: FrameNode, values: number[], W: number, H: number): void {
...
function drawSankey(plot: FrameNode, W: number, H: number): void {
  const sources = [
    { label: "A", weight: 0.5 },
    { label: "B", weight: 0.3 },
    { label: "C", weight: 0.2 },
  ...
  const layoutCol = (col: { label: string; weight: number }[], x: number) => {
...
async function handleInsertTable(payload: InsertTablePayload): Promise<void> {
  await loadFonts();
  ...
  notify(
    "status",
    `Inserted table (${payload.columns.length} columns, ${payload.rows} rows)${
      ds.tableComponent ? "" : " · fallback (library Table not found)"
    }.`,
  );
}

async function buildLibraryTable(payload: InsertTablePayload): Promise<FrameNode> {
...
function findCellVariantPropertyName(): string | null {
  const set = ds.tableComponent;
  if (!set) return null;
  const defs =
    "componentPropertyDefinitions" in set
      ? set.componentPropertyDefinitions
      : null;
  if (!defs) return null;
  for (const name of Object.keys(defs)) {
    if (TABLE_CELL_TYPE_PROPERTY_MATCHERS.some((re) => re.test(name))) return name;
  }
  return null;
}

function findHeaderPropertyName(): string | null {
  const set = ds.tableComponent;
  if (!set) return null;
  const defs =
    "componentPropertyDefinitions" in set
  ...
  if (!defs) return null;
  for (const name of Object.keys(defs)) {
    if (TABLE_HEADER_PROPERTY_MATCHERS.some((re) => re.test(name))) return name;
  }
  return null;
}

async function instantiateTableRow(
  columns: { header: string; type: ColumnType }[],
  kind: "header" | "body",
  cellVariantProperty: string | null,
  headerProperty: string | null,
  rowIndex = 0,
): Promise<FrameNode> {
  ...
  for (const col of columns) {
    ...
    const defaultVariant = cellSource.type === "COMPONENT_SET"
      ...
      : cellSource;
    const instance = (defaultVariant as ComponentNode).createInstance();

    const props: { [k: string]: string | boolean } = {};
    if (cellVariantProperty) {
      const variantValue = resolveVariantValueForType(col.type, cellVariantProperty);
      if (variantValue != null) props[cellVariantProperty] = variantValue;
    }
    if (headerProperty) {
...
function resolveVariantValueForType(type: ColumnType, propertyName: string): string | null {
  const set = ds.tableComponent;
  if (!set) return null;
  const defs =
    "componentPropertyDefinitions" in set
  ...
  if (!def || def.type !== "VARIANT" || !def.variantOptions) return null;
  const hints = VARIANT_VALUE_HINTS[type];
  for (const opt of def.variantOptions) {
    if (hints.some((re) => re.test(opt))) return opt;
  }
  return def.variantOptions[0] || null;
}

function setFirstTextInNode(node: SceneNode, value: string): void {
  if (node.type === "TEXT") {
  ...
  if ("findAll" in node) {
    const texts = (node as ChildrenMixin).findAll((n) => n.type === "TEXT") as TextNode[];
    if (texts.length > 0) {
      const target = texts[0];
      figma.loadFontAsync(target.fontName as FontName).then(() => {
...
async function buildFallbackTable(payload: InsertTablePayload): Promise<FrameNode> {
  const container = figma.createFrame();
...
function mockValue(type: ColumnType, rowIndex: number): string {
  switch (type) {
    case "Text":
...
function notify(type: "ready" | "status" | "error", detail: string): void {
